...
class EngineU_net:
    ...
    def export_all_transformations(self, dcm_dir, mat_path, output_dir, threshold=0.5):
        ...
        def compute_pred_mask():
            ct_volume = np.stack(ct_slices, axis=0)  # (6, H, W)
            pred_mask = self.predict_from_numpy(ct_volume)
            
            # Приводим к формату (num_slices, H, W) если нужно
            # Если pred_mask имеет форму (H, W, 6) — транспонируем
            if pred_mask.ndim == 3 and pred_mask.shape[-1] == self.n_slices:
                pred_mask = np.transpose(pred_mask, (2, 0, 1))
            
            # Если pred_mask имеет форму (H, W) — это один срез, дублируем на все 6
            elif pred_mask.ndim == 2:
                pred_mask = np.stack([pred_mask] * self.n_slices, axis=0)
            
            # Теперь pred_mask точно (6, H, W)
        ...
        def save_current_state(state_name):
            ...
            total_active = pred_binary.sum()
            print(f"  📊 {state_name}: активированных пикселей = {total_active}")
            for idx in range(self.n_slices):
                slice_active = pred_binary[idx].sum()
                print(f"     срез {idx}: {slice_active} пикселей")
