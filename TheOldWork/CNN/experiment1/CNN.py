...
class CNN(nn.Module):
    def __init__(self):
        ...
        #первый входной слой сверточный  сети, на вход принимает 1 параметр, на выход 
        #дает 64 признака 
        ...
        # Второй сверточный слой: 64 → 128 фильтра
        # принцип тот же принять 64 параметра, найти признаки, вернуть 128 параметра
        ...
        #слой побольше)))) значит получше))))
        self.conv5 = nn.Conv2d(512, 1024, kernel_size=3, padding=1)
        #БАМ! не ожидали?
        #плохое решение для задачи класификации букв
        #оказалось что оно адаптивно усредняет всю матрицу, это конечно плюс -
        #так как адаптивно плющит тензор - при подаче в полносвязный слой не вохникнет ошибок, но мы теряем локальные признаки
        ...
        #H_in - размер входной стороны (ширина или высота) картинки или карты признаков.
        #padding - сколько пикселей добавляется по краям обычно 0 или 1
        #kernel_size - размер свёрточного фильтра
        #stride - шаг свёртки (по сколько пикселей фильтр двигается).
        self.fc1 = nn.Linear(1024*8*8, 512)
        #dropout была попытка воткнуть но что то я не могу расчитать его верное значение 
        #dropout ставится перед полносвязными слоями, особенно если fc большой.
        #Conv → Pool → Flatten → fc1 → Dropout → fc2 → Dropout → fc3 → Output.
    ...
    def forward(self, x):
        ...
        #Выравнивание
        #TODO интересно, если ставить #self.pool = nn.AdaptiveAvgPool2d((x,x)) нужно ли выравнивание?
        x = x.view(x.size(0), -1)
